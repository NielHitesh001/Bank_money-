...
export default function RealTimeCandleChart({ symbol = "EUR/USD" }) {
  ...
  // Chart Geometry
  const rsiHeight = showRSI ? 45 : 0;
  const padding = { top: 12, right: 48, bottom: 18, left: 8 };
  const mainChartHeight = Math.max(130, dimensions.height - rsiHeight - 60);
  const chartWidth = Math.max(340, dimensions.width - 16);
  ...
  return (
    <div ref={containerRef} className="terminal-candle-panel" style={{ width: "100%", height: "300px", minHeight: "300px", maxHeight: "300px", display: "flex", flexDirection: "column", boxSizing: "border-box" }}>
      {/* Header Controls Bar */}
      <div className="candle-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "nowrap", gap: "6px", marginBottom: "2px" }}>
        <div className="candle-title-group" style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <div>
            <span className="eyebrow" style={{ fontSize: "8.5px", color: "#64dcb1", letterSpacing: "0.5px" }}>
              LIVE TICKER STREAM
            </span>
            <h3 style={{ margin: 0, fontSize: "14px", color: "#f0fdf4", display: "flex", alignItems: "center", gap: "6px" }}>
              {symbol}
              <span style={{ fontSize: "12px", color: isOverallUp ? "#52d6aa" : "#ff5b6e" }}>
