    ...
    (outgoing.get(current.nodeId) || []).forEach((transaction) => {
      if (!visited.has(transaction.target)) {
        ...
        queue.push({
          ...
          edges: [...current.edges, transaction.id],
          nodes: [...current.nodes, transaction.target],
        });
      }
    });
  }
...
export function findBidirectionalPath(transactions, startId, endId) {
  if (!startId || !endId) return { nodeIds: [], edgeIds: [] };
  if (startId === endId) return { nodeIds: [startId], edgeIds: [] };
  ...
  transactions.forEach((tx) => {
    // Add forward
    const outEdges = adj.get(tx.source) || [];
    outEdges.push({ neighbor: tx.target, edgeId: tx.id, forward: true, tx });
    adj.set(tx.source, outEdges);

    // Add backward
    const inEdges = adj.get(tx.target) || [];
    inEdges.push({ neighbor: tx.source, edgeId: tx.id, forward: false, tx });
    adj.set(tx.target, inEdges);
  });
  ...
  for (let cursor = 0; cursor < queue.length; cursor += 1) {
    ...
    if (current.nodeId === endId) {
      return { nodeIds: current.nodes, edgeIds: current.edges };
    }
    (adj.get(current.nodeId) || []).forEach(({ neighbor, edgeId }) => {
